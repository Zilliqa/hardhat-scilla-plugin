...
describe("", function () {
  ...
  describe("Contract connect", function () {
    ...
    it("Should be able to connect to a contract", async function () {
      const contract = await this.hre.interactWithScillaContract(
    ...
    it("Should be able to call a contract", async function () {
      ...
      await expect(result).to.have.eventLogWithParams("getHello()", {
        value: testMessage,
        vname: "msg",
      });
    });
    ...
    it("Should return undefined when we try to connect to a contract which doesn't exist", async function () {
      const contract2 = await this.hre.interactWithScillaContract(
        "0x95D302877382c871681852bc2f87d56f41dC7aF2"
      );
